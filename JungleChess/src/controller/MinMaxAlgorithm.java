...
public class MinMaxAlgorithm {
    ...
    private static final ChessboardPoint BLUE_DEN_POINT = new ChessboardPoint(8, 3); // 蓝方兽穴位置
    private static final ChessboardPoint RED_DEN_POINT = new ChessboardPoint(0, 3); // 红方兽穴位置
            ...
            for (ChessboardPoint piece : getRedPieces(board)) {
                ...
                if(board.getChessPieceAt(piece).getRank()==6||board.getChessPieceAt(piece).getRank()==7) {
                    ...
                    for (int[] dir : LTdir) {
                        ...
                        newPoint = new ChessboardPoint(newX, newY);
                        if (!repeatedMove(piece,newPoint)&&validMove(board, piece, newPoint)) {
    ...
    private static boolean repeatedMove(ChessboardPoint piece, ChessboardPoint newPoint) {
    ...
    private static int evaluate(Chessboard board) {
        ...
        for(ChessboardPoint P : getBluePieces(board)){
            ChessPiece piece = board.getChessPieceAt(P);
            int rank = piece.getRank();
            score -= rank*blueTable[P.getRow()][P.getCol()];
        }
        return score;
    }
